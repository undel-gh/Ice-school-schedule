...
# Validation deliberately distinguishes obsolete *ownership* from field names that
# are still valid on the new entities.  In particular, `visit_limit_snapshot` is
# correct on SubscriptionAllowance and therefore must NOT be globally forbidden.
FORBIDDEN = [
    'SubscriptionUsage',
    'category_snapshot',
...
REQUIRED_BY_DOC = {
    # Domain and Django schema documents describe plan-level allowances explicitly.
    'domain': [
        'SubscriptionPlanAllowance',
        'SubscriptionAllowance',
        'AttendanceCoverage',
    ...
    # State machines operate on issued/runtime entitlements; they do not need to
    # mention the catalog-side SubscriptionPlanAllowance entity.
    'state': [
        'SubscriptionAllowance',
        'AttendanceCoverage',
        'OneTimeEntitlement',
...
def check_consolidated(contents: dict[str,str]) -> list[str]:
    ...
    for key in ('domain','state','django','spec'):
        ...
        for term in FORBIDDEN:
            if term in body:
                errors.append(f'{key}: obsolete term remains: {term}')

        # `visit_limit_snapshot` remains valid only as a property of
        # SubscriptionAllowance.  Catch common regressions where it is accidentally
        # reintroduced on Subscription itself without rejecting the legitimate
        # allowance field.
        if re.search(
            r'(?s)Subscription(?!Allowance).*?visit_limit_snapshot',
            body,
        ):
            # Limit the match to the same model/code block where possible.  The
            # broad regex above can cross prose, so confirm by searching for the
            # legacy field inside an explicit Subscription definition.
            legacy_patterns = [
                r'(?s)Subscription\n(?:(?!SubscriptionAllowance).){0,700}\bvisit_limit_snapshot\b',
                r'(?s)class Subscription\b(?:(?!class SubscriptionAllowance).){0,1600}\bvisit_limit_snapshot\b',
            ]
            if any(re.search(pattern, body) for pattern in legacy_patterns):
                errors.append(
                    f'{key}: obsolete Subscription.visit_limit_snapshot remains'
                )

        for term in REQUIRED_BY_DOC[key]:
            if term not in body:
                errors.append(f'{key}: required term missing: {term}')
...
def main():
    ...
    # Build the patch with Git itself rather than difflib.
    #
    # Four source Markdown files in the repository currently have no trailing
    # newline at EOF.  difflib.unified_diff() does not emit Git's
    # "\\ No newline at end of file" marker, which can produce a syntactically
    # corrupt patch.  `git diff` handles EOF markers, spaces and Unicode paths
    # correctly, so use it as the canonical patch generator.
    patch = ""
    target_keys = ('domain', 'state', 'django', 'spec')
    target_paths = [str(FILES[key]).replace('\\\\', '/') for key in target_keys]

    if not args.apply or args.output:
        # Temporarily materialize the transformed documents, ask Git to diff
        # them against HEAD/index, and always restore the exact original bytes.
        try:
            ...
            for key in target_keys:
                FILES[key].write_bytes(raw[key])

        if not patch:
            raise SystemExit('Generated patch is empty.')

        # Validate the exact bytes that will be written/shown.
        ...
        if check.returncode != 0:
            raise SystemExit(
                'Internal git apply --check failed for the generated patch:\\n'
                + check.stderr.decode('utf-8', errors='replace')
            )

    if args.output:
        Path(args.output).write_bytes(patch.encode('utf-8'))
